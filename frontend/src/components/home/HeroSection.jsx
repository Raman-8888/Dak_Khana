...
import { ALL_VIDEO_URLS, getVideoSrc } from '../../lib/videoCache';
...
export default function HeroSection() {
  ...
  // Ensure active video plays on mount/remount, even if cached
  useEffect(() => {
    const activeEl = activeLayer === 'a' ? videoARef.current : videoBRef.current;
    if (activeEl) {
      activeEl.muted = true;
      ...
      <div className="hero__container">
        ...
        <div className="hero__player" ref={playerRef} id="hero-player">
          <div className="hero__player-frame">
            ...
            <video
              ref={videoARef}
              src={getVideoSrc(srcA)}
              className={`hero__player-video ${activeLayer === 'a' ? 'hero__player-video--active' : ''}`}
              muted
              playsInline
              autoPlay
              onLoadedData={(e) => { e.target.muted = true; e.target.playbackRate = 1.3; if (activeLayer === 'a') e.target.play().catch(() => {}); }}
            ...
            <video
              ref={videoBRef}
              src={srcB ? getVideoSrc(srcB) : undefined}
              className={`hero__player-video ${activeLayer === 'b' ? 'hero__player-video--active' : ''}`}
              muted
              playsInline
              preload="auto"
              onLoadedData={(e) => { e.target.muted = true; e.target.playbackRate = 1.3; }}
