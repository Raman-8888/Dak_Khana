...
import { getVideoSrc } from '../../lib/videoCache';
...
export default function VideoCircle({ videoSrc, config, index }) {
  ...
  const handleTimeUpdate = useCallback((e) => {
    const src = activeSlot === 'A' ? slotASrc : slotBSrc;
    if (!src) return;
    if (src.includes('/3.mp4') || src.includes('/3.mp3')) {
      if (e.target.currentTime >= 3) {
        e.target.currentTime = 0;
      }
    } else if (src.includes('/4.mp4')) {
