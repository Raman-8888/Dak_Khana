...
class AssignmentController extends Controller
{
    ...
    {
        $available = ExportRequest::query()
            ->with(['receiverDetail', 'packageDetail', 'senderDetail'])
            ->whereNotIn('status', ['delivered', 'cancelled', 'failed'])
            ->whereHas('payments', function ($query) {
                $query->where('status', 'successful');
            })
            ->whereDoesntHave('deliveryAssignments', function ($query) {
                $query->whereNotIn('status', ['cancelled', 'failed', 'delivered']);
            })
    ...
    public function selfAssign(Request $request, string $id)
    {
        $export = ExportRequest::query()->findOrFail($id);
        ...
        if ($activeAssignment) {
            $agentName = $activeAssignment->agent->name ?? 'another agent';
            return response()->json([
                'success' => false,
                'message' => 'Already assigned to ' . $agentName,
            ], 409);
        }
        ...
        $export->trackingEvents()->create([
            'status'   => 'processing',
            'location' => 'Operations',
            'notes'    => 'Self-assigned by agent '.$request->user()->name,
        ]);

        return response()->json([
            'success' => true,
            'message' => 'Assignment successful.',
            'data'    => $assignment->load('shipment'),
        ], 201);
    }
    ...
    public function pickup(Request $request, int $id)
    {
        ...
        $assignment->shipment->trackingEvents()->create([
            ...
            'notes'    => 'Picked up by agent '.$request->user()->name.'. OTP for delivery is: '.$otp,
        ]);
